...
class Controller_En_Home extends Controller_Application {
	...
	public function action_index()
	{
		...
		$page = ORM::factory('Page')->where('deleted', '=', 'false')->and_where('title', '=', 'Home')->find();
	...
	public function action_search($level = '', $program = '')
	{
		...
		$total_program = ORM::factory('Certificate_Program')->where('deleted', '=', 'false')->and_where('certificate_id', '=', $level_id)->and_where('title', 'like', '%'.$course.'%')->count_all();
		...
		$popular = ORM::factory('Certificate_Program')->where('deleted', '=', 'false')->and_where('popular', '=', 'yes')->order_by('sort', 'asc')->limit(4)->find_all();

		$programs = ORM::factory('Certificate_Program')->where('deleted', '=', 'false')->and_where('certificate_id', '=', $level_id)->and_where('title', 'like', '%'.$course.'%')->offset($pagination->offset)->limit($pagination->items_per_page)->order_by('sort', 'desc')->find_all();
		...
		$view = View::factory($this->lang.'/search')
			...
			->bind('total_program', $total_program);
