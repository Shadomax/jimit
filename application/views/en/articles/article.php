                    ...
                    <div class="col-xs-12 col-sm-8 post_left">
                        <div class="post_left_section post_left_border">
                            <div class="post single_post">
                                ...
                                <div class="meta">
                                    <span class="author">By: <a href="#"><?=strip_tags($article->posted_by)?></a></span>
                                    <span class="category"> <a href="#"><?=strip_tags($article->category->name)?></a></span>
                                    <span class="date">Posted: <a href="#"><?=date('F', $article->datetime)?> <?=date('d', $article->datetime)?>, <?=date('Y', $article->datetime)?></a></span>
                                </div><!--end meta-->
                            ...
                            <div class="related_post_sec single_post">
                                ...
                                <div class="alert alert-info" role="alert">
                                    <strong>Heads up!</strong> No related post
                                </div>
                            <?php endif;?>
                                <ul>
                                <?php foreach ($cats as $key) :?>
                                    <li>
                                        <span class="rel_thumb">
                                            <a href="<?=URL::site('articles/article'.'/'.$key->id.'/'.URL::title($key->title, '_'))?>"><img src="<?=$key->getPicture()?>" alt="<?=$key->title?>"></a>
                                        </span><!--end rel_thumb-->
                                        <div class="rel_right">
                                            <h4><a href="<?=URL::site('articles/article'.'/'.$key->id.'/'.URL::title($key->title, '_'))?>"><?=strip_tags($key->title)?></a></h4>
                                            <div class="meta">
                                                <span class="author">By: <a href="<?=URL::site('articles/article'.'/'.$key->id.'/'.URL::title($key->title, '_'))?>"><?=strip_tags($key->posted_by)?></a></span>
                                                <span class="category"> <a href="<?=URL::site('articles/article'.'/'.$key->id.'/'.URL::title($key->title, '_'))?>"><?=strip_tags($key->category->name)?></a></span>
                                                <span class="date">Posted: <a href="<?=URL::site('articles/article'.'/'.$key->id.'/'.URL::title($key->title, '_'))?>"><?=date('F', $key->datetime)?> <?=date('d', $key->datetime)?>, <?=date('Y', $key->datetime)?></a></span>
                                            </div>
                                            <p>
                            ...
                            <div class="comments_section">
                                ...
                                <div class="comments_form">
                                    ...
                                    <div class="alert alert-warning" role="alert">
                                        <strong>Warning!</strong> Some errors were encountered. Fix them to continue.
                                    </div>
                                  <?php endif;?>
                                    <div class="half">
                                    ...
                                    <div class="full">
                                      <textarea rows="9" id="comment" cols="10" name="comment" class="form-control" placeholder="Write a comment"></textarea>
                            ...
                            <div class="list_block">
                                ...
                                <?php foreach ($articles as $article) :?>
                                    <li>
                                        ...
                                        <div class="rel_right">
                                            <a href="<?=URL::site('articles/article'.'/'.$article->id.'/'.URL::title($article->title, '_'))?>"><h4><?=strip_tags($article->title)?></h4></a>
                                            <span class="date">Posted: <a href="<?=URL::site('articles/article'.'/'.$article->id.'/'.URL::title($article->title, '_'))?>"><?=date('F', $article->datetime)?> <?=date('d', $article->datetime)?>, <?=date('Y', $article->datetime)?></a></span>
                                        </div><!--end rel right-->
                                    </li>
                                <?php endforeach;?>
                                </ul>
                                <a href="<?=URL::site('articles')?>" class="more_post">More</a>
                            </div><!-- end list_block -->
